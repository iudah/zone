...
int32_t *randint(uint64_t n_int, int64_t hi, int64_t lo) {
  srand(seed);
  int32_t *iptr = zcalloc(n_int, sizeof(*iptr));
  for (uint64_t i = 0; i < n_int; i++) {
    iptr[i] = lo + (int32_t)(((float)random() * (hi - lo)) / (float)RAND_MAX);
  }
  return iptr;
}
void get_batch(float *split, uint64_t split_len, uint64_t batch_size,
               uint64_t block_size, float **data) {
  int32_t *ix = randint(batch_size, split_len - batch_size - 1, 0);
  float *data_x = data[0] = zcalloc(batch_size * block_size, sizeof(*data_x));
  float *data_y = data[1] = zcalloc(batch_size * block_size, sizeof(*data_x));
  for (uint64_t i = 0; i < batch_size; i++) {
    for (uint64_t j = 0; j < block_size; j++) {
      data_x[i * block_size + j] = split[ix[i] + j];
      data_y[i * block_size + j] = split[ix[i] + 1 + j];
    }
  }
}
...
int main() {
  ...
  auto learning = zn_supervised_learning(network, zn_cross_entropy, zn_sgd);
